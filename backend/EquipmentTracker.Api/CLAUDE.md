...
- **Ремонты** (`Repair` + `RepairOperationItem` + `RepairPartItem`, `RepairsController`,
  `POST/GET /api/equipment-units/{id}/repairs`): фиксирует Operator+, обязательна ≥1 операция (части и примечание опциональны); списание расходных частей
  идёт в той же транзакции, наличие НЕ проверяется — остаток может стать отрицательным (так задумано).
  Автор хранится снимком (`AccountLogin`). Удаление единицы техники каскадно удаляет её ремонты;
  операции/части, использованные в ремонтах, удалить нельзя. В самих `Repair` истории
  редактирования нет — она есть у `SparePart` (там видно списание).
